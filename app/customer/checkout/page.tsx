...
const generatePDF = () => {
  ...
  // Customer Info
  doc.setFontSize(12);
  doc.text(`Name: ${checkoutData.name}`, 14, 35);
  doc.text(`Phone: ${checkoutData.delivery_phone}`, 14, 42);
  doc.text(`Address: ${checkoutData.delivery_address}`, 14, 49);

  doc.text(`Order Date: ${new Date().toLocaleString()}`, 14, 60);

  // Items Table
  const tableRows = checkoutData.items.map((item) => [
    item.name,
    item.quantity,
    `₹${item.price}`,
  ...
  doc.text(`Subtotal: ₹${checkoutData.subtotal}`, 14, finalY);
  doc.text(`Delivery Fee: ₹${checkoutData.delivery_fee}`, 14, finalY + 7);

  if (checkoutData.discount > 0) {
    doc.text(`Discount: -₹${checkoutData.discount}`, 14, finalY + 14);
  }
  ...
  doc.text(
    ...
    14,
    finalY + (checkoutData.discount > 0 ? 25 : 20)
  );
  ...
  try {
    const response = await fetch("http://localhost:8000/api/coupons/validate", {
      ...
      body: JSON.stringify({
        ...
        orderTotal: checkoutData.subtotal + checkoutData.delivery_fee,
      }),
    });
    ...
    // Update checkout data with new total
    const newTotal = Math.max(
      0,
      checkoutData.subtotal + checkoutData.delivery_fee - discountAmount
    );
    ...
    setCouponSuccess(`✅ ${coupon.description} - You save ₹${discountAmount.toFixed(2)}`);
    setCouponCode("");
  } catch (err: any) {
...
const handleRemoveCoupon = () => {
  ...
  if (checkoutData) {
    setCheckoutData({
      ...checkoutData,
      discount: 0,
      total: checkoutData.subtotal + checkoutData.delivery_fee,
    });
  }
};
        ...
        <div>
          <Card className="p-6 sticky top-4">
            ...
            <div className="border-t pt-4 mb-4">
              ...
              {appliedCoupon ? (
                <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 p-3 rounded-lg mb-3">
                  <div className="flex justify-between items-start">
                    ...
                    <button
                      ...
                      className="text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 text-sm font-semibold"
                    >
                      Remove
                    </button>
                  </div>
                ...
                <form onSubmit={handleApplyCoupon} className="space-y-2">
                  <div className="flex gap-2 items-center">
                    <input
                      ...
                      className="flex-1 min-w-0 border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                      disabled={couponLoading}
                    />
                    <button
                      type="submit"
                      disabled={couponLoading || !couponCode.trim()}
                      className="shrink-0 px-3 py-2 bg-primary text-primary-foreground rounded text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {couponLoading ? "..." : "Apply"}
                    </button>
                  </div>
