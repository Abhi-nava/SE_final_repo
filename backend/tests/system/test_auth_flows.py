...
async def test_complete_registration_and_login_flow(client_and_db):
    ...
    verify_payload_1 = {"temp_token": temp_token_1, "otp": otp_1}
    ...
    verify_payload_2 = {"temp_token": temp_token_2, "otp": otp_2}
...
async def test_complete_password_reset_flow(client_and_db):
    ...
    verify_res = await client.post(
        "/api/auth/forgot-password/verify", json=verify_payload
    )
...
async def test_2fa_token_expiry_behavior(client_and_db):
    ...
    temp_token = TwoFactorAuthToken.create_temp_token(
        str(user_id), "expiry@example.com", expires_in=1
    )
...
async def test_forgot_password_invalid_payloads(client_and_db):
    ...
    res3 = await client.post(
        "/api/auth/forgot-password/reset", json={"new_password": "newpass"}
    )
