...
async def register(user_data: UserCreate):
    ...
    temp_token = TwoFactorAuthToken.create_temp_token(
        user_id=str(result.inserted_id), email=user_data.email
    )
    ...
    if token_data:
        ...
        try:
            ...
            logger.warning(
                f"Failed to send OTP email (signup) to {user_data.email}: {e}"
            )
...
async def request_otp(email: str):
    ...
    try:
        ...
        logger.warning(
            f"Failed to send OTP email (password reset) to {email}: {e}. OTP: {otp}"
        )
...
async def forgot_password_start(payload: Dict[str, Any]):
    ...
    if token_data and user:
        ...
        try:
            ...
            logger.warning(
                f"Failed to send OTP email (forgot-password) to {email}: {e}"
            )
...
async def forgot_password_verify(payload: Dict[str, Any]):
    ...
    if not reset_temp_token or not otp:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="reset_temp_token and otp are required",
        )
...
async def forgot_password_reset(payload: Dict[str, Any]):
    ...
    if not reset_session_token or not new_password:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="reset_session_token and new_password are required",
        )
