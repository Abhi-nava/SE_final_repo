...
async def get_agent_stats(agent_id: str):
    ...
    completed_orders = await db.orders.count_documents(
        {"delivery_agent_id": str(agent["_id"]), "order_status": "delivered"}
    )
...
async def assign_order(order_id: str):
    ...
    await db.orders.update_one(
        ...
        {
            "$set": {
                "delivery_agent_id": str(available_agent["_id"]),
    ...
    await db.delivery_agents.update_one(
        {"_id": available_agent["_id"]}, {"$set": {"status": "on_delivery"}}
    )
...
async def register_delivery_agent(
    agent_data: DeliveryAgentCreate, current_user=Depends(get_current_delivery_agent)
):
    """Register as delivery agent with KYC details"""
    # Check if already registered
    existing_agent = await db.delivery_agents.find_one(
...
async def update_agent_profile(
    update_data: DeliveryAgentUpdate, current_user=Depends(get_current_delivery_agent)
):
...
async def update_location(
    location_data: LocationUpdate, current_user=Depends(get_current_delivery_agent)
):
    """Update delivery agent's current location"""
    agent = await db.delivery_agents.find_one({"user_id": str(current_user["_id"])})
...
async def get_available_orders(current_user=Depends(get_current_delivery_agent)):
    ...
    if not agent or not agent.get("is_verified"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Agent not verified"
        )

    orders = (
        await db.orders.find(
            {"status": OrderStatus.READY.value, "delivery_agent_id": None}
        )
        .limit(10)
        .to_list(10)
    )
...
async def accept_delivery(
    order_id: str, current_user=Depends(get_current_delivery_agent)
):
...
async def update_delivery_status(
    order_id: str, status: str, current_user=Depends(get_current_delivery_agent)
):
...
async def get_my_deliveries(current_user=Depends(get_current_delivery_agent)):
    """Get all deliveries for agent"""
    agent = await db.delivery_agents.find_one({"user_id": str(current_user["_id"])})

    orders = (
        await db.orders.find({"delivery_agent_id": str(agent["_id"])})
        .sort("created_at", -1)
        .to_list(None)
    )
