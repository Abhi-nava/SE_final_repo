...
async def get_analytics_overview(current_user = Depends(get_current_admin)):
    ...
    try:
        ...
        active_delivery_agents = await db.delivery_agents.count_documents({"is_active": True})
...
async def get_all_users(
    ...
    try:
        users = await db.users.find({"role": "customer"}).skip(skip).limit(limit).to_list(limit)
