...
def test_system_password_reset_with_otp_flow(fake_db):
    """System test: User forgets password -> Requests OTP -> Resets password -> Logs in with new password"""
    ...
    # 2) User requests OTP
    asyncio.run(auth_module.request_otp(user_data.email))
    
    # 3) Verify OTP was generated and stored
    otp_record = asyncio.run(fake_db.otp_tokens.find_one({"email": user_data.email}))
    assert otp_record is not None
    assert len(otp_record["otp"]) == 6
    
    # 4) User resets password with OTP
    new_password = "NewPassword456"
    reset_data = auth_module.PasswordReset(
        email=user_data.email,
        otp=otp_record["otp"],
        new_password=new_password
    )
    result = asyncio.run(auth_module.reset_password(reset_data))
    ...
    login_resp = asyncio.run(auth_module.login(
        auth_module.LoginRequest(email=user_data.email, password=new_password)
    ))
    assert login_resp.access_token
...
def test_system_otp_expiration_enforcement(fake_db):
    """System test: OTP generated -> Time passes -> OTP expires -> Reset fails"""
    # 1) Create user
    user_data = UserCreate(
        email="expire_otp@example.com",
        phone="4445556666",
        full_name="OTP Expire User",
    ...
    # 3) Manually expire the OTP
    asyncio.run(fake_db.otp_tokens.update_one(
        {"email": user_data.email},
        {"$set": {"expires_at": datetime.now(timezone.utc) - timedelta(minutes=1)}}
    ))
    
    # 4) Try to reset password with expired OTP
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(auth_module.reset_password(reset_data))
